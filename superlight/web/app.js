...
const words = {
  en:{connecting:"Connecting",online:"Collector online",offline:"Collector offline",eyebrow:"A LITTLE CLARITY",title:"Usage, at a glance.",subtitle:"Your tokens. Your models. A clearer picture.",connect:"Connect Codex",overview:"Overview",rates:"Model rates",local:"Stays on your Mac",day:"Today",week:"This week",month:"This month",estimate:"Estimated API value in USD. Not your subscription bill.",activity:"Spending activity",estimated:"Estimated cost",byModel:"By model",modelHint:"A breakdown of the selected period.",export:"Export ↗",model:"Model",input:"Input",cached:"Cached",output:"Output",cost:"Est. cost",emptyTitle:"Ready when you are.",emptyHint:"Connect Codex and finish a response to see your first record.",emptyPeriod:"No usage recorded in this period.",cacheNote:"Cached tokens are part of input, not extra tokens.",referenceRates:"Reference rates",rateUnit:"USD per 1 million tokens",longContext:"Long context",howCost:"A simple, transparent estimate.",formula:"Uncached input × input rate + cached input × cache rate + output × output rate.",exclusions:"Excludes tools, Fast mode, regional uplifts and cache-write premiums. Unknown prices stay unknown.",waiting:"Waiting for the collector…",refresh:"Refresh",openDashboard:"Open dashboard ↗",oneTime:"ONE-TIME SETUP",setupTitle:"Bring your usage into the light.",setupHint:"Add this section to your user config at ~/.codex/config.toml. Merge an existing [otel] section instead of adding it twice.",copyConfig:"Copy configuration",setupAfter:"Restart Codex, then complete a local task. Your existing login and model connection stay the same.",setupScope:"Local Codex tasks only. Remote tasks need a collector on their execution host.",copied:"Configuration copied",copyFailed:"Select and copy the configuration above.",tokens:"tokens",responses:"responses",updated:"Updated",asOf:"Price snapshot",partial:"unpriced responses excluded from the estimate",missing:"responses with missing token counts",unknown:"Unknown",error:"Cannot reach SuperLight. Start the collector, then refresh.",standard:"Standard rate",threshold:"input tokens",partialLabel:"Partial estimate",exported:"Summary exported",noData:"No recorded usage",settings:"Collector settings"},
  zh:{connecting:"正在连接",online:"服务运行中",offline:"服务未连接",eyebrow:"本机用量 · 清楚可见",title:"用量，一眼明了。",subtitle:"记录每次调用，看清 Token 与参考成本。",connect:"接入 Codex",overview:"用量总览",rates:"模型单价",local:"数据留在本机",day:"今日",week:"本周",month:"本月",estimate:"按 API 单价估算，单位 USD，不代表订阅实际扣费。",activity:"消费走势",estimated:"估算消费",byModel:"模型明细",modelHint:"所选时段的用量与参考成本。",export:"导出 ↗",model:"模型",input:"输入",cached:"缓存",output:"输出",cost:"估算金额",emptyTitle:"等待第一条记录。",emptyHint:"接入 Codex，完成一次响应后即可查看用量。",emptyPeriod:"这个时段还没有用量记录。",cacheNote:"缓存已计入输入，不额外累加。",referenceRates:"模型参考单价",rateUnit:"每百万 Token 的美元价格",longContext:"长上下文",howCost:"每一笔估算，都有依据。",formula:"非缓存输入 × 输入单价 + 缓存输入 × 缓存单价 + 输出 × 输出单价。",exclusions:"不含工具调用、Fast 模式、区域附加费和缓存写入溢价。未知单价不会记为零。",waiting:"等待接收器…",refresh:"刷新",openDashboard:"打开完整面板 ↗",oneTime:"一次配置，持续记录",setupTitle:"让用量显示在这里。",setupHint:"将以下内容加入用户配置 ~/.codex/config.toml。如果已有 [otel]，请合并修改，不要重复添加。",copyConfig:"复制配置",setupAfter:"重启 Codex，然后完成一次本机任务。沿用现有登录和模型网络连接。",setupScope:"适用于本机 Codex 任务。远程任务需在执行主机配置接收器。",copied:"配置已复制",copyFailed:"请选中上方配置并复制。",tokens:"Token",responses:"次响应",updated:"更新于",asOf:"价格核对日期",partial:"条响应缺少估价，金额仅含已知部分",missing:"条响应缺少完整 Token 计数",unknown:"未知",error:"暂时无法连接 SuperLight。请启动接收器后刷新。",standard:"标准单价",threshold:"输入 Token",partialLabel:"部分估算",exported:"汇总已导出",noData:"尚无用量记录",settings:"接收器设置"}
};
...
function render(){
  for(const p of ["day","week","month"]){const a=data.periods[p].totals;$(p+"-cost").textContent=money(a.estimated_cost_usd);$(p+"-tokens").textContent=number(a.total_tokens)+" "+t("tokens")+(a.unpriced_responses?" · "+t("partialLabel"):"");}
  const selected=data.periods[period], totals=selected.totals;
  $("coverage").hidden=!totals.unpriced_responses&&!totals.missing_usage_responses;
  $("coverage").textContent=[totals.unpriced_responses?`${totals.unpriced_responses} ${t("partial")}`:"",totals.missing_usage_responses?`${totals.missing_usage_responses} ${t("missing")}`:""].filter(Boolean).join(" · ");
